...
def plot_results2(solvers, solver_names, s1_ins_throu, s2_ins_throu, s3_ins_throu, figname):
    ...
    # Fig. 2: Rate over time.
    fig2 = plt.figure(figsize=(14, 4))
    fig2.subplots_adjust(bottom=0.3, wspace=0.3)

    ax2 = fig2.add_subplot(111)
    ax2.plot(range(len(solvers[0].actions)), [s1_ins_throu[x] for x in solvers[0].actions], '-',label=solver_names[0])
    ax2.plot(range(len(solvers[1].actions)), [s2_ins_throu[x] for x in solvers[1].actions], '-',label=solver_names[1])
    ax2.plot(range(len(solvers[2].actions)), [s3_ins_throu[x] for x in solvers[2].actions], '-',label=solver_names[2])
    ax2.set_xlabel('Time step')
    ax2.set_ylabel('Instantaneous rate (Mbps)')
    ax2.grid('k', ls='--', alpha=0.3)
    plt.savefig('Instantaneous rate.png',dpi=600)
...
def experiment3(N,K,rate_set,probas1,probas2,probas3):
    ...
    s_throu1 = list(map(lambda e,f:e*f, probas1,rate_set))
    s_throu2 = list(map(lambda e,f:e*f, probas2,rate_set))
    s_throu3 = list(map(lambda e,f:e*f, probas3,rate_set))
    

    s1_ins_throu = [s_throu1[i] for i in s1.actions[:int(N/3)]] + [s_throu2[i] for i in s1.actions[int(N/3)+1:int(2*N/3)]] + [s_throu3[i] for i in s1.actions[int(2*N/3)+1:]]
    s2_ins_throu = [s_throu1[i] for i in s2.actions[:int(N/3)]] + [s_throu2[i] for i in s2.actions[int(N/3)+1:int(2*N/3)]] + [s_throu3[i] for i in s2.actions[int(2*N/3)+1:]]
    s3_ins_throu = [s_throu1[i] for i in s4.actions[:int(N/3)]] + [s_throu2[i] for i in s4.actions[int(N/3)+1:int(2*N/3)]] + [s_throu3[i] for i in s4.actions[int(2*N/3)+1:]]

    plot_results2(test_solvers_mod, names, s1_ins_throu, s2_ins_throu, s3_ins_throu, "results_N{}_K{}_time-varing.png".format(N,K))
...
if __name__ == '__main__':
    ...
    experiment(5000,20,rate_set,probas)
    ...
    #experiment3(12000,20,rate_set,probas1,probas2,probas3)